export interface Dictionary {
  lobby: {
    connecting: string;
    invited: string;
    joinRoomInfo: string;
    displayName: string;
    enterNamePlaceholder: string;
    enterNameShort: string;
    enterGame: string;
    createOwnRoom: string;
    returnToHome: string;
    gameLobbyTitle: string;
    gameNames: {
      whoKnow: string;
      gobbler: string;
      ticTacToe: string;
      handDuel: string;
      soundsFishy: string;
    };
    or: string;
    roomCodePlaceholder: string;
    join: string;
    publicLobbies: string;
    host: string;
    playersInRoom: string;
    roomHost: string;
    unknownHost: string;
    copyLink: string;
    qrCode: string;
    leave: string;
    players: string;
    score: string;
    offline: string;
    you: string;
    locked: string;
    waitingRoom: string;
    hostSelection: string;
    roundRobin: string;
    random: string;
    roomCreatorFixed: string;
    timerMinutes: string;
    minutes: string;
    mode: string;
    oneVOneRoundRobin: string;
    allAtOnce: string;
    targetScore: string;
    bo1: string;
    bo3: string;
    bo5: string;
    bo7: string;
    bestOf: string;
    startGame: string;
    waitingMin: string;
    waitingForHost: string;
    phase: string;
    secretWordSelection: string;
    youAreHost: string;
    enterSecretWordDesc: string;
    typeSecretWord: string;
    confirmSecretWord: string;
    leaveRoomTitle: string;
    leaveRoomDesc: string;
    cancel: string;
    leaveRoomBtn: string;
    invitePlayers: string;
    scanQrCodeDesc: string;
    close: string;
  };
  errors: {
    enterNameFirst: string;
    inviteLinkCopied: string;
  };
  rules: {
    button: string;
    modal: {
      title: string;
      tabs: {
        overview: string;
        whoKnow: string;
        gobbler: string;
        ticTacToe: string;
        handDuel: string;
        soundsFishy: string;
      };
      closeBtn: string;
    };
    lobby: {
      welcomeTitle: string;
      welcomeDesc: string;
      modesTitle: string;
      whoKnowDesc: string;
      gobblerDesc: string;
      ticTacToeDesc: string;
      handDuelDesc: string;
      soundsFishyDesc: string;
    };
    whoKnow: {
      setupTitle: string;
      setupDesc1: string;
      setupDescGameHost: string;
      setupDesc2: string;
      setupDescInsider: string;
      setupDesc3: string;
      setupDescCommoners: string;
      phasesTitle: string;
      phase1Title: string;
      phase1Desc: string;
      phase2Title: string;
      phase2Desc: string;
      phase3Title: string;
      phase3Desc: string;
      winCondTitle: string;
      winCond1A: string;
      winCond1B: string;
      winCond1C: string;
      winCond2A: string;
      winCond2B: string;
      winCond2C: string;
      winCond3A: string;
      winCond3B: string;
    };
    gobbler: {
      title: string;
      desc: string;
      rulesTitle: string;
      pieceSizesTitle: string;
      pieceSizesDesc: string;
      gobblingTitle: string;
      gobblingDesc1: string;
      gobblingDesc2: string;
      gobblingDesc3: string;
      movingTitle: string;
      movingDesc: string;
      winningTitle: string;
      winningDesc: string;
    };
    ticTacToe: {
      title: string;
      desc: string;
      winTitle: string;
      winDesc1A: string;
      winDesc1B: string;
      winDesc1C: string;
      winDesc2A: string;
      winDesc2B: string;
    };
    rps: {
      title: string;
      desc: string;
      basicsTitle: string;
      beats: string;
      modesTitle: string;
      mode1v1Title: string;
      mode1v1Desc: string;
      modeAllTitle: string;
      modeAllDesc: string;
    };
    soundsFishy: {
      title: string;
      desc: string;
      rolesTitle: string;
      rolePicker: string;
      rolePickerDesc: string;
      roleBlueFish: string;
      roleBlueFishDesc: string;
      roleRedHerring: string;
      roleRedHerringDesc: string;
      flowTitle: string;
      flowSetup: string;
      flowPitch: string;
      flowHunt: string;
      flowScore: string;
      scoringTitle: string;
      scoreRedHerring: string;
      scoreBank: string;
      scoreBlueFish: string;
    };
  };
  gameSoundsFishy: {
    loading: string;
    yourRole: string;
    rolePicker: string;
    roleBlueFish: string;
    roleRedHerring: string;
    currentPot: string;
    pts: string;
    setupPhase: string;
    theTopic: string;
    trueAnswer: string;
    waitingForFish: string;
    answerSubmitted: string;
    waitingForOthers: string;
    mustEnterTrue: string;
    mustEnterFake: string;
    typeAnswerPlaceholder: string;
    submitAnswer: string;
    otherFishesTyping: string;
    revealEveryoneFirst: string;
    allRevealedEliminate: string;
    revealAnswer: string;
    eliminateLooksFishy: string;
    eliminated: string;
    youHavePointsPrefix: string;
    youHavePointsSuffix: string;
    bankPointsAndEnd: string;
    roundOver: string;
    howEveryoneScored: string;
    pickerText: string;
    blueFishText: string;
    redHerringText: string;
    plusPointsPrefix: string;
    plusPointsSuffix: string;
    playNextRound: string;
  };
  gameDetectiveMath: {
    loading: string;
    yourRole: string;
    yourScore: string;
    roleInformer: string;
    roleConspirator: string;
    roleDetective: string;
    pts: string;
    setupPhase: string;
    informerSetupDesc: string;
    waitingInformerSetup: string;
    yourHand: string;
    enterTargetNumber: string;
    targetNumberPlaceholder: string;
    confirmNumber: string;
    waitingInformer: string;
    playingPhase: string;
    round: string;
    targetNumberIs: string;
    conspiratorDoesNotKnow: string;
    playCardPrompt: string;
    waitingFor: string;
    playedCards: string;
    discussionPhase: string;
    discussionDesc: string;
    votingPhase: string;
    whoIsConspirator: string;
    conspiratorWait: string;
    votes: string;
    lockedOn: string;
    lockVote: string;
    scoringPhase: string;
    conspiratorWas: string;
    youCaughtConspirator: string;
    conspiratorEscaped: string;
    youWereCaught: string;
    youEscaped: string;
    playNextRound: string;
    waitingHostNextRound: string;
  };
}
