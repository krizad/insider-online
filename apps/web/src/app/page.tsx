...
import { DetectiveMathView } from "@/components/games/detective-math/DetectiveMathView";
      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 bg-slate-950 text-slate-200 relative">
        ...
        <div className="w-full max-w-md p-6 sm:p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl">
          ...
          <div className="space-y-6">
            ...
            <div className="mb-3">
              <button onClick={() => createRoom(GameType.DETECTIVE_MATH)} disabled={!myName} className="w-full bg-indigo-800/80 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-colors shadow-lg border border-indigo-600/50 flex flex-col items-center justify-center gap-1 group">
                <span className="text-xl group-hover:scale-110 transition-transform">🔢</span>
                <span className="text-xs tracking-wider text-center px-1">Detective Math</span>
              </button>
            </div>
    ...
    <main className="flex min-h-[100dvh] flex-col items-center p-2 sm:p-4 bg-slate-950 text-slate-200">
      <div className="w-full max-w-4xl mx-auto flex flex-col gap-2 sm:gap-4 flex-1 relative">
        ...
        {room.gameType === GameType.GOBBLER_TIC_TAC_TOE ? (
          ...
          <DetectiveClubView />
        ) : room.gameType === GameType.DETECTIVE_MATH && room.status !== RoomStatus.LOBBY ? (
          <DetectiveMathView />
        ) : (
